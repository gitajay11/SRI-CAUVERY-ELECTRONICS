 ...
 *
 * Every page is rendered on the server and most of them query the database,
 * so a navigation takes as long as it takes — and until it lands the browser
 * shows the *old* page, unchanged. On a phone on a slow connection that gap
 * runs to seconds, and the honest reading of a screen that does not move is
 * that the tap missed. People tap again.
 *
 * Deliberately not a root `loading.tsx`. That is the framework's own answer
 * and would be the better one, except that a loading boundary flushes the
 * response — and its 200 — before the page runs, so product and category
 * pages that call `notFound()` would start answering 200 for things that do
...
export function NavigationProgress() {
  ...
  return (
    <div
      // Purely an announcement: it must never swallow a tap on the page
